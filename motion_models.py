...
class MM_CTRV(MotionModel):
    ...
    def initialize_state(self, z0: np.ndarray, z1: np.ndarray, T:float):
        # initialize state using 2 measurement, following 2 point-diff initialization
        x = np.zeros(self.dim_state)
        x[IX], x[IY] = z1[0], z1[1]
        # vx, vy = (z1[0] - z0[0])/T, (z1[1] - z0[1])/T
        # x[IPHI] = np.arctan2(vy, vx)
        # x[IV] = np.sqrt(vx**2 + vy**2)
        return x
